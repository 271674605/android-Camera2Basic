...
死锁的条件
   ...
   解决方法：
1.调整申请锁的范围 ：同步方法改成同步代码块，同步代码块缩小代码块范围。
2.调整申请锁的顺序：当多个线程需要相同的一些锁，但是按照不同的顺序加锁，死锁就很容易发生。如果能确保所有的线程都是按照相同的顺序获得锁，那么死锁就不会发生。
3：加锁时限：在尝试获取锁的时候加一个超时时间，这也就意味着在尝试获取锁的过程中若超过了这个时限该线程则放弃对该锁请求。使用显式Lock锁，在获取锁时使用tryLock()方法。当等待超过时限的时候，tryLock()不会一直等待，而是返回错误信息。
4：死锁检测：JDK提供了两种方式来给我们检测：
